...
class StreamPETRHead(AnchorFreeHead):
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            ...
            self.memory_egopose = data['ego_pose_inv'].unsqueeze(1) @ self.memory_egopose  # equation 8 
            self.memory_reference_point = transform_reference_points(self.memory_reference_point, data['ego_pose_inv'], reverse=False)# equation 9 
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        if self.training and mask_dict and mask_dict['pad_size'] > 0:
            rec_reference_points = all_bbox_preds[:, :, mask_dict['pad_size']:, :3][-1]  # output of last decoder layer
    ...
    def position_embeding(self, data, memory_centers, topk_indexes, img_metas):
        ...
        coords3d[..., 0:3] = (coords3d[..., 0:3] - self.position_range[0:3]) / (self.position_range[3:6] - self.position_range[0:3]) # norm 0~1
        coords3d = coords3d.reshape(B, -1, D*3) # a ray has D point ,each point contains xyz
      
        pos_embed  = inverse_sigmoid(coords3d) # inv norm 
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training and self.with_dn:
            ...
            # add noise
            groups = min(self.scalar, self.num_query // max(known_num))  # unused 
            known_indice = known_indice.repeat(self.scalar, 1).view(-1)  # indice repeat 10x 
            known_labels = labels.repeat(self.scalar, 1).view(-1).long().to(reference_points.device) # label repeat 10x
            ...
            if self.bbox_noise_scale > 0:
                ...
                rand_prob = torch.rand_like(known_bbox_center) * 2 - 1.0 # a uniform distribution [0,1). x2 -1?
                known_bbox_center += torch.mul(rand_prob,
                                            diff) * self.bbox_noise_scale  
                known_bbox_center[..., 0:3] = (known_bbox_center[..., 0:3] - self.pc_range[0:3]) / (self.pc_range[3:6] - self.pc_range[0:3]) # norm 0 ~ 1

                known_bbox_center = known_bbox_center.clamp(min=0.0, max=1.0)
                mask = torch.norm(rand_prob, 2, 1) > self.split
                known_labels[mask] = self.num_classes # noise label
            
            single_pad = int(max(known_num))
            pad_size = int(single_pad * self.scalar)
            padding_bbox = torch.zeros(pad_size, 3).to(reference_points.device)
            padded_reference_points = torch.cat([padding_bbox, reference_points], dim=0).unsqueeze(0).repeat(batch_size, 1, 1) # cat with init rp
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        pos_embed, cone = self.position_embeding(data, memory_center, topk_indexes, img_metas)  # 3d PE

        memory = self.memory_embed(memory)
        memory = self.spatial_alignment(memory, cone) # focal petr  equation 7
        pos_embed = self.featurized_pe(pos_embed, memory)  # petrv2 Fig.4  . f2d_encode * pe3d + f2d = 3d PE 
        ...
        query_pos = self.query_embedding(pos2posemb3d(reference_points)) # dim = 128*3 384 -> query_embed 256
