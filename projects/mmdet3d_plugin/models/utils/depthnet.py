...
class CameraAwareDepthNet(nn.Module):
    ...
    def forward(self, x, intrinsics, extrinsics):
        ...
        context_se = self.context_mlp(
            mlp_input)[..., None, None]  # (B*N_view, C_mid, 1, 1)
...
class SELikeModule(nn.Module):
    def __init__(self, in_channel=256, feat_channel=256, intrinsic_channel=6):
        ...
        self.input_conv = nn.Conv2d(
            in_channel, feat_channel, kernel_size=1, padding=0)
...
@DepthNet.register_module()
class CameraAwareDepthNetV3(nn.Module):
    def __init__(self, in_channels, mid_channels, context_channels, depth_channels, num_params1=18,
                 num_params2=6, with_depth_correction=False, with_context_encoder=False, fix_alpha=None,
                 with_pgd=False):
        super(CameraAwareDepthNetV3, self).__init__()
        self.in_channels = in_channels
        # self.context_channels = context_channels
        self.depth_channels = depth_channels
        self.mid_channels = mid_channels
